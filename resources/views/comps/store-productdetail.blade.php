    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
            <div class="pro-detail w-full flex flex-col justify-center order-last lg:order-none max-lg:max-w-[608px] max-lg:mx-auto">
                <p class="font-medium text-lg text-indigo-600 mb-4">Colecciones &nbsp; / &nbsp; {{ $category }}</p>
                <h2 class="mb-2 font-manrope font-bold text-3xl leading-10 text-gray-900">{{ $product['name'] }}</h2>
                    ...
                    <div class="block w-full">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
                            <div class="flex items-center justify-center w-full">
                                <button id="qtyless" class="group py-4 px-6 border border-gray-400 rounded-l-full shadow-sm shadow-transparent transition-all duration-500 hover:shadow-gray-300 hover:bg-gray-50">
                                ...
                                <form id="postCart" action="{{ route('cart.add') }}" method="POST">
                                    @csrf
                                    <input type="text" id="qty" name="qty" min="1" value="1" class="font-semibold text-gray-900 text-lg py-[13px] px-6 w-full lg:max-w-[118px] border-y border-gray-400 bg-transparent placeholder:text-gray-900 text-center hover:bg-gray-50 focus-within:bg-gray-50 outline-0" placeholder="1">
                                    <input type="hidden" name="id" value="{{ $product['id'] }}">
                                    <input type="hidden" name="name" value="{{ $product['name'] }}">
                                    <input type="hidden" name="price" value="{{ $product['price'] }}">
                                    <input type="hidden" name="image" value="{{ $product['image'] }}">
                                </form>
                                <button id="qtymore" class="group py-4 px-6 border border-gray-400 rounded-r-full shadow-sm shadow-transparent transition-all duration-500 hover:shadow-gray-300 hover:bg-gray-50">
                                    <svg class="stroke-gray-700 transition-all duration-500 group-hover:stroke-black" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button id="addToCart" class="group py-4 px-5 rounded-full bg-indigo-50 text-indigo-600 font-semibold text-lg w-full flex items-center justify-center gap-2 shadow-sm shadow-transparent transition-all duration-500 hover:bg-indigo-100 hover:shadow-indigo-200">
...
@push('scripts')
    <script>
        var sumqty = document.getElementById('qtymore');
        var lessqty = document.getElementById('qtyless');
        var qtyinput = document.getElementById('qty');

        sumqty.addEventListener('click', function() {
            var qty = Number(qtyinput.value);
            qty += 1;
            qtyinput.value = qty.toString();
        });

        lessqty.addEventListener('click', function() {
            var qty = Number(qtyinput.value);
            if(qty > 1) {
                qty -= 1;
                qtyinput.value = qty.toString();
        ...
        var formCart = document.getElementById('postCart');
