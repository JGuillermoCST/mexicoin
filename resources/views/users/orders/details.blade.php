{{--
|--------------------------------------------------------------------------
| Componente: Tabla de órdenes con modal de detalle (Tailwind CSS)
|--------------------------------------------------------------------------
| Uso:
|   @include('components.orders-table', ['orders' => $orders])
|
| Requiere Tailwind CSS instalado en el proyecto.
|
| Estructura esperada de cada orden:
|   $order->id, ->name, ->email, ->payment_method,
|   ->status, ->total, ->created_at  (opcional pero recomendado)
--}}
...
@php
    ...
    $methodConfig = [
        ...
        'oxxo'     => ['label' => 'OXXO',          'icon' => '🏪'],
    ];

    $statusConfig = [
        'completed' => ['label' => 'Completado',  'class' => 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200'],
        'pending'   => ['label' => 'Pendiente',   'class' => 'bg-amber-50 text-amber-700 ring-1 ring-amber-200'],
        'failed'    => ['label' => 'Fallido',     'class' => 'bg-red-50 text-red-700 ring-1 ring-red-200'],
        'refunded'  => ['label' => 'Reembolsado', 'class' => 'bg-violet-50 text-violet-700 ring-1 ring-violet-200'],
    ];
...
{{-- ── Tabla ── --}}
<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden" id="ot-wrap">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-slate-100">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <select
                id="ot-status-filter"
                onchange="otFilter()"
                class="h-9 px-3 text-sm border border-slate-200 rounded-lg bg-slate-50 text-slate-600 outline-none focus:border-slate-800 transition cursor-pointer"
            >
